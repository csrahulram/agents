...
| 2 | **Tests written by a small model are weak** (test strictness moved our own results by 30 points — the largest effect measured) — 29–60% defect detection; 7B models produced only crash-detection | [[test-quality]] | Tests from fixed examples, written by a different model, plus mutation testing | **Open** — target: catch 8 of 10 injected bugs |
...
| 4 | **Retries degrade small models** — self-conditioning on their own errors | [[error-compounding]], [[repair-loops]] | Fresh context per attempt, code-written feedback, candidates over repairs, cap 2–3 | **Measured 2026-09-23:** fresh context beats carried by 10 points in both scorings; candidates beat retries by 20. [[loop-mechanisms-measured]] |
| 5 | **Compounding maths** — 95% per card gives 21% on a 30-card project | [[error-compounding]] | Goal is cheap recovery, not high per-card success; blocked cards are the real metric | **Quantified:** ~80% per card with all mechanisms → ~6 blocked cards per 30-card project. Open as a question of tolerance |
